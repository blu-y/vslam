...
int main(int argc, char **argv) {
    // The Eigen/Geometry module provies a variety of rotation and translation representations
    ...
    AngleAxisd rotation_vector(M_PI / 4, Vector3d(0,0,1)); // Rotate 45 degrees along Z axis
    cout.precision(3);
    cout << "rotation vector: \n angle = " << rotation_vector.angle() << "\t axis = " << rotation_vector.axis().transpose() << endl;
    ...
    Vector3d euler_angles = rotation_matrix.eulerAngles(2, 1, 0); // ZYX order, ie rpy order
    ...
    v_transformed = rotation_matrix * v + Vector3d(1, 3, 4);
    cout << "R*v+t = " << v_transformed.transpose() << endl;
    ...
    auto q = Quaterniond(rotation_vector);
