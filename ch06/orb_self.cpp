...
int main(int argc, char **argv) {
    ...
    FAST(img_1, keypoints_1, 40);
    FAST(img_2, keypoints_2, 40);
    ComputeORB(img_1, keypoints_1, descriptors_1);
    ComputeORB(img_2, keypoints_2, descriptors_2);
    chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
    auto time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "compute ORB cost = " << time_used.count() << " seconds." << endl;

    // --brute-force matching
    vector<DMatch> matches;
    t1 = chrono::steady_clock::now();
    BfMatch(descriptors_1, descriptors_2, matches);
    ...
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int k = 0; k < 32; k++) {
                ...
                // compare pp and qq
                if (img.at<uchar>(pp.y, pp.x) < img.at<uchar>(qq.y, pp.x)) {
                    d |= 1 << k; // d <- d|(OR)1
                }
            }
            desc[i] = d;
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<DMatch> &matches) {
    ...
    for (int i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            int distance = 0;
            for (int k = 0; k < 8; k++) {
                distance += popcount(desc1[i1][k] ^desc2[i2][k]);
            }
