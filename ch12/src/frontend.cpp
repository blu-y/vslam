...
namespace mySLAM {
    ...
    bool Frontend::InsetKeyframe() {
        ...
        // update backend with new keyframe
        backend_->UpdateMap();
    ...
    int Frontend::TriangulateNewPoints() {
        vector<SE3> poses{camera_left_->pose(), camera_right_->pose()};
        ...
        for (size_t i = 0; i < current_frame_->features_left_.size(); ++i) {
            if (current_frame_->features_left_[i]->map_point_.expired() &&
                ...
                // the matching points are on the right image.
                // Try triangulation.
                vector<Vec3> points{
                ...
                if (triangulation(poses, points, pworld) && pworld[2] > 0) {
                    ...
                    map_->InsertKeyFrame(new_map_point);
    ...
    int Frontend::EstimateCurrentPose() {
        ...
        // estimate Pose, and determine outliers
    ...
    int Frontend::TrackLastFrame() {
        ...
        LOG(INFO) << "Found " << num_good_pts << " in last image.";
        return num_good_pts;
    }
    ...
    int Frontend::DetectFeatures() {
        Mat mask(current_frame_->left_img_.size(), CV_8SC1, 255);
        for (auto &feat:current_frame_->features_left_) {
    ...
    int Frontend::FindFeaturesInRight() {
        ...
        LOG(INFO) << "Found " << num_good_pts << " in right image.";
        return num_good_pts;
    }
    ...
    bool Frontend::Reset() {
        LOG(INFO) << "Frontend::Reset TODO" << endl;
        // StereoInit();
        return true;
    }
