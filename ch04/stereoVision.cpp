...
typedef vector<Vector4d, Eigen::aligned_allocator<Vector4d>> PointCloudType;
...
int main(int argc, char **argv) {
    ...
    // change v++ and u++ to v+=2, v+=2 if your machine is slow to get a sparser cloud
...
void showPointCloud(const PointCloudType &pointcloud) {
    ...
    while (pangolin::ShouldQuit() == false) {
        ...
        for (auto &p: pointcloud) {
            glColor3f(p[3], p[3], p[3]); // depth as color
