...
void DirectPoseEstimationSingleLayer(const Mat &img1, const Mat &img2,
                                     const VecVector2d &px_ref,
                                     const vector<double> depth_ref,
                                     Sophus::SE3d &T21);
void DirectPoseEstimationMultiLayer(const Mat &img1, const Mat &img2,
                                    const VecVector2d &px_ref,
                                    const vector<double> depth_ref,
                                    Sophus::SE3d &T21);
...
int main(int argc, char **argv) {
    ...
    // randomly pick pixels in the first image and generate 3d points in the first image's frame
    RNG rng;
    int nPoints = 2000;
    int boarder = 20;
    ...
    // estimates 01~05.png's pose using this info
...
void JacobianAccumulator::accumulate_jacobian(const Range &range) {
    // parameters
    ...
    for (size_t i = range.start; i < range.end; i++) {
        ...
        for (int x = -half_patch_size; x <= half_patch_size; x++)
            for (int y = -half_patch_size; y <= half_patch_size; y++) {
                ...
                J_img_pixel = Vector2d(0.5 * (GetPixelValue(img2, u+1+x, v+y)
    ...
    if (cnt_good) {
        ...
        unique_lock<mutex> lock(hessian_mutex);
        H += hessian;
        b += bias;
...
void DirectPoseEstimationSingleLayer(const Mat &img1, const Mat &img2,
    ...
    for (int iter = 0; iter < iterations; iter++) {
        jaco_accu.reset();
        parallel_for_(Range(0, px_ref.size()),
            bind(&JacobianAccumulator::accumulate_jacobian, &jaco_accu, std::placeholders::_1));
        ...
        if (update.norm() < 1e-3) break; // converge
    ...
    Mat img2_show;
    cvtColor(img2, img2_show, COLOR_GRAY2BGR);
    VecVector2d projection = jaco_accu.projected_points();
    for (size_t i = 0; i < px_ref.size(); i++) {
        auto p_ref = px_ref[i];
        auto p_cur = projection[i];
        if (p_cur[0] > 0 && p_cur[1] > 0) {
            circle(img2_show, Point2f(p_cur[0], p_cur[1]), 2, Scalar(0, 250, 0), 2);
            line(img2_show, Point2f(p_ref[0], p_ref[1]), Point2f(p_cur[0], p_cur[1]), Scalar(0, 250, 0));
        }
    }
